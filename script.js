...
let id = parseInt( localStorage.getItem('lastId') ?? 0 ) + 1;
let food = [
    {
        name:`Rawon`,
        stok: 0,
    ...
    },
    {
        ...
        stok: 10,
        harga: 13000,
        image: './assets/images/pindanh.jpg'
    },
        {
        name:`Nasi Rames`,
        stok: 14,
        harga: 11000,
        image: './assets/images/ramesan.jpeg'
    },
    {
        name:`Gorengan`,
        stok: 50,
...
function orderFood(){
    ...
    if(checkAvailable()){
        ...
        // data dalam json
        const record = {
            pesanan : '',harga : totalHargaMakanan, id
        };
        
        
        // loop ke setiap makanan yang di pesan
        cart.forEach(e=>{
            record.pesanan += `${e.name}, `
        });
        
        
        // ambil data sebelumnya dan jadikan json
        const dataSebelumya = JSON.parse(localStorage.getItem('data') ?? '[]');
        dataSebelumya.push(record);
        
        // tulis lagi dengan data yang baru
        localStorage.setItem('data', JSON.stringify(dataSebelumya));
